module Quickcheck: Bastet.Test.QUICKCHECK
  with type t = Alcotest.test_case<unit>
  and type arbitrary<'a> = QCheck.arbitrary<'a> = {
  ...
  let property3 = (~count=default_count, ~name=default_name, arb_a, arb_b, arb_c, cb) =>
    QCheck.Test.make(~count, ~name, QCheck.triple(arb_a, arb_b, arb_c), ((a, b, c)) =>
      cb(a, b, c)
    ) |> QCheck_alcotest.to_alcotest

  let property4 = (~count=default_count, ~name=default_name, arb_a, arb_b, arb_c, arb_d, cb) =>
    QCheck.Test.make(~count, ~name, QCheck.quad(arb_a, arb_b, arb_c, arb_d), ((a, b, c, d)) =>
      cb(a, b, c, d)
    ) |> QCheck_alcotest.to_alcotest
}
