...
describe("Promise", () => {
  ...
  describe("Monad (unlawful)", () => {
    ...
    async_property1(
      ...
      arb_nat,
      \"<."(
        \"<."(Obj.magic, V.associativity(\"<."(pure, string_of_int), \"<."(pure, \"^"("!")))),
        promise,
      ),
    )
    async_property1(
    ...
    async_property1("will *seemingly* properly flatten (not correct)", arb_nat, n => {
      open Promise.Infix
      \">>="(Fn.flatten(pure(pure(n))), flattened_n => pure(flattened_n == n))
    })
    async_property1("promises are not actually nested despite the type signature", arb_nat, n => {
      open Promise.Infix
      /* This next line throws a runtime error because the inner value is not a promise */

      \">>="(
        \">>="(pure(pure(n)), result => pure(\">>="(result, \"<."(pure, Function.const(false))))),
